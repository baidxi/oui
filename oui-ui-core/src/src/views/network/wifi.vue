        ...
        <el-tab-pane v-for="radio in radios" :key="radio.name" :name="radio.name" :label="radio.name + ` (${radio.hardware})`">
            <uci-form config="wireless" :apply-timeout="15">
                ...
                <uci-section :title="$t('Interface')" type="wifi-iface" :options="{radio: radio.name}" :filter="filterInterface" addable :after-add="afterAddIface">
                    ...
                    <uci-tab :title="$t('Associated Stations')" name="station">
                       <uci-option name="ifname" :load="update_assoclist">
...
export default {
  ...
  methods: {
    ...
    update_assoclist(sid, self) {
       const ifname = this.$uci.get('wireless', sid, 'ifname');
      this.$ubus.call('iwinfo', 'assoclist', {'device':ifname}).then(r => {
        r.results.map(r => {
          this.assoclist[ifname] = r;
        })
      });
    },
